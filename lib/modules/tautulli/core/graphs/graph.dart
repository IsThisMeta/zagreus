...
class TautulliGraphHelper {
  ...
  FlBorderData borderData() => FlBorderData(
        show: true,
        border: Border.all(color: ZagColours.white10),
      );
      ...
      return data.categories![value.truncate()]!.length > maxTitleLength + 1
          ? [
              data.categories![value.truncate()]!
                  ...
                  .toUpperCase(),
              if (titleOverFlowShowEllipsis) ZagUI.TEXT_ELLIPSIS,
            ].join()
    ...
    return FlTitlesData(
      ...
      bottomTitles: AxisTitles(
        sideTitles: SideTitles(
          showTitles: true,
          reservedSize:
              ZagUI.FONT_SIZE_GRAPH_LEGEND + ZagUI.DEFAULT_MARGIN_SIZE,
          getTitlesWidget: (value, meta) {
            return Padding(
              padding: const EdgeInsets.only(top: ZagUI.DEFAULT_MARGIN_SIZE),
              child: Text(
                _getTitle(value),
                style: const TextStyle(
                  color: ZagColours.grey,
                  fontSize: ZagUI.FONT_SIZE_GRAPH_LEGEND,
                ),
              ),
            );
          },
          ...
          (index) => Padding(
            child: Row(
              children: [
                Padding(
                  child: Container(
                    height: ZagUI.FONT_SIZE_GRAPH_LEGEND,
                    width: ZagUI.FONT_SIZE_GRAPH_LEGEND,
                    decoration: BoxDecoration(
                      color: ZagColours().byGraphLayer(index),
