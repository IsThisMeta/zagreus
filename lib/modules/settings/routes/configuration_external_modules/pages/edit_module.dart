...
class _State extends State<ConfigurationExternalModulesEditRoute>
  ...
  ZagExternalModule? _module;

  @override
  Widget build(BuildContext context) {
    if (widget.moduleId < 0 ||
        !ZagBox.externalModules.contains(widget.moduleId)) {
  ...
  Widget _body() {
    return ZagBox.externalModules.listenableBuilder(
      ...
      builder: (context, dynamic _) {
        if (!ZagBox.externalModules.contains(widget.moduleId))
          return Container();
        _module = ZagBox.externalModules.read(widget.moduleId);
        return ZagListView(
