...
class _State extends State<TMDBPopularTVShowsRoute>
  ...
  void initState() {
    ...
    final cached = DiscoverSessionCache().get('TMDBPopularTVShowsRoute');
    if (cached != null) {
  ...
  Widget _showTile(Map<String, dynamic> show) {
    ...
    return GestureDetector(
      ...
      onLongPress: _isMultiSelectMode ? null : () => _showTVShowPreview(show),
      child: titlesBeneath
                    ...
                    Positioned(
                      ...
                      child: Container(
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            color:
                                _ratingColor((show['rating'] ?? 0.0).toDouble()),
                            fontSize: 10,
                            fontWeight: FontWeight.bold,
                          ),
                        ),
  ...
  /// or Sonarr actions dialog for items already in library
  Future<void> _showTVShowPreview(Map<String, dynamic> show) async {
    ...
    await ZagDialogs().textPreviewWithAdd(
      ...
      title,
      overview,
      onAdd: () => _openSeriesInSonarrQuick(tmdbId: tmdbId, title: title),
      alignLeft: true,
  ...
  Future<void> _showSonarrSeriesActions({
    ...
    try {
      ...
      showZagSnackBar(
        title: seriesTitle ?? 'Sonarr',
        message: 'Could not load series actions.',
        type: ZagSnackbarType.ERROR,
      );
    }
  }

  Future<void> _openSeriesInSonarrQuick({int? tmdbId, String? title}) async {
    final sonarrState = context.read<SonarrState>();
    if (!sonarrState.enabled || sonarrState.api == null) {
      showZagSnackBar(
        title: title ?? 'Sonarr',
        message: 'Connect Sonarr to add shows from Dashboard.',
        type: ZagSnackbarType.INFO,
      );
      return;
    }
    ...
    try {
      ...
      dismissLoader();
      if (!mounted) return;
      ZagLogger().error('Failed to open Sonarr add series flow', error, stack);
      showZagSnackBar(
        title: title ?? 'TV Show',
        message: 'Something went wrong talking to Sonarr.',
        type: ZagSnackbarType.ERROR,
