...
class MovieGenreDiscoverRoute extends StatefulWidget {
  final int genreId;
  final String genreName;
  final String? backdropUrl;

  const MovieGenreDiscoverRoute({
    Key? key,
  ...
  @override
  State<MovieGenreDiscoverRoute> createState() => _State();
}
...
class _State extends State<MovieGenreDiscoverRoute> with ZagScrollControllerMixin {
  ...
  void initState() {
    ...
    final cacheKey = 'MovieGenreDiscoverRoute_${widget.genreId}';
  ...
  void dispose() {
    if (_movies.isNotEmpty) {
      DiscoverSessionCache().set(
        'MovieGenreDiscoverRoute_${widget.genreId}',
        DiscoverRouteState(
          items: _movies,
  ...
  Future<void> _loadMovies({bool silent = false}) async {
    ...
    try {
      ...
      final movies = await TMDBApi.getMoviesByGenre(
        widget.genreId,
        page: 1,
        region: region,
      );
      ...
      ZagLogger().error('Failed to load movies for genre', error, stack);
      if (!mounted) return;
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              'Error Loading ${widget.genreName} Movies',
  ...
  Future<void> _openMovieInRadarr({int? tmdbId, String? title}) async {
    ...
    try {
      ...
      if (radarrState.movies != null) {
        ...
        if (match == null && lowerTitle != null && lowerTitle.isNotEmpty) {
          for (final movie in radarrMovies) {
