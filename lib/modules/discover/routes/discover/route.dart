...
class _State extends State<DiscoverHomeRoute> with ZagScrollControllerMixin {
  ...
  // Add extra height for titles beneath - scales with text size
  double _posterListHeightFor(BuildContext context) {
    final baseHeight = _posterHeight + 8.0;
    if (_showTitles) {
      final textScaler = MediaQuery.textScalerOf(context);
      // Base 32px for 2-line title (fontSize 13 * ~1.2 line height * 2 lines) + 6px spacing
      // Scales with system text size
      final titleHeight = 38.0 * textScaler.scale(1.0);
      return baseHeight + titleHeight;
    }
    return baseHeight;
  }

  // Convenience getter for places that don't have context - uses default scale
  double get _posterListHeight => _posterHeight + 8.0 + (_showTitles ? 38.0 : 0.0);
  ...
  Future<void> _loadTrendingData() async {
    try {
      ...
      // Show carousel immediately after TMDB data arrives
      if (mounted) {
        setState(() {
        ...
        // Start auto-scroll now that data is ready (only on first load)
        if (!_autoScrollStarted) {
          _autoScrollStarted = true;
          _startAutoScroll();
        }

        // Precache hero images in background (non-blocking)
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (!mounted) return;
      ...
      for (final show in shows) {
        ...
        if (tmdbId != null) {
          ...
          if (tmdbDetails != null) {
            ...
            show['backdrop'] =
                TMDBApi.getImageUrl(tmdbDetails['backdrop_path'], size: 'w780');
      ...
      for (final movie in movies) {
        ...
        if (tmdbId != null) {
          ...
          if (details != null) {
            ...
            movie['backdrop'] = TMDBApi.getImageUrl(details['backdrop_path'], size: 'w780');
