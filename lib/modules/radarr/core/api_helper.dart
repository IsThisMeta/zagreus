  ...
  Future<RadarrMovie?> addMovie({
    ...
    if (context.read<RadarrState>().enabled) {
      final result = await context
        ...
        if (showSnackbar)
          showZagErrorSnackBar(title: 'Failed to Add Movie', error: error);
        return RadarrMovie();
      });
  ...
  Future<bool> backupDatabase({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context.read<RadarrState>().api!.command.backup().then((_) {
        if (showSnackbar)
          showZagSuccessSnackBar(
              title: 'Backing Up Database${ZagUI.TEXT_ELLIPSIS}',
              message: 'Backing up the database in the background');
        return true;
      }).catchError((error, stack) {
        ZagLogger().error('Unable to backup database', error, stack);
        if (showSnackbar)
          showZagErrorSnackBar(
  ...
  Future<bool> missingMovieSearch({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        if (showSnackbar)
          showZagSuccessSnackBar(
              title: 'Searching${ZagUI.TEXT_ELLIPSIS}',
  ...
  Future<bool> runRSSSync({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context.read<RadarrState>().api!.command.rssSync().then((_) {
        if (showSnackbar)
          showZagSuccessSnackBar(
              title: 'Running RSS Sync${ZagUI.TEXT_ELLIPSIS}',
              message: 'Running RSS sync in the background');
        return true;
      }).catchError((error, stack) {
        ZagLogger().error('Unable to run RSS sync', error, stack);
        if (showSnackbar)
          showZagErrorSnackBar(title: 'Failed to Run RSS Sync', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> updateLibrary({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        if (showSnackbar)
          showZagSuccessSnackBar(
              title: 'Updating Library${ZagUI.TEXT_ELLIPSIS}',
              message: 'Updating library in the background');
        return true;
      }).catchError((error, stack) {
        ZagLogger().error('Unable to update library', error, stack);
        if (showSnackbar)
          showZagErrorSnackBar(
  ...
  Future<bool> deleteMovieFile({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        if (showSnackbar)
          showZagErrorSnackBar(title: 'Failed to Delete File', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> updateMovie({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        ZagLogger().error('Failed to update movie: ${movie.id}', error, stack);
        showZagErrorSnackBar(title: 'Failed to Update Movie', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> removeMovie({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        ZagLogger().error('Failed to remove movie: ${movie.id}', error, stack);
        showZagErrorSnackBar(title: 'Failed to Remove Movie', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> quickImport({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        if (showSnackbar)
          showZagSuccessSnackBar(
            title: 'Running Quick Import${ZagUI.TEXT_ELLIPSIS}',
            message: path,
          );
        return true;
      }).catchError((error, stack) {
        ...
        showZagErrorSnackBar(title: 'Failed to Quick Import', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> deleteTag({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        ZagLogger().error('Failed to add tag: ${tag.id}', error, stack);
        if (showSnackbar)
          showZagErrorSnackBar(title: 'Failed to Delete Tag', error: error);
        return false;
      });
    }
    return false;
